...
function createIncludeFile($ret){
	...
	for($i=0;$i<count($ret);$i++){
		...
		$str .= "\trequire_once('class/mysql/".$clazzName."BD.class.php');\n";
		$str .= "\trequire_once('class/mysql/ext/".$clazzName."BDExt.class.php');\n";
	}
...
function createDAOFactory($ret){
	...
	for($i=0;$i<count($ret);$i++){
		...
		$str .= "\t\treturn new ".$clazzName."BDExt();\n";
		$str .= "\t}\n\n";
	}
...
function getnerateDomainObjects($ret){
	for($i=0;$i<count($ret);$i++){
		...
		for($j=0;$j<count($tab);$j++){
			$fields .= "\t\tprivate $".getVarNameWithS($tab[$j][0]).";\n\r";
		}
